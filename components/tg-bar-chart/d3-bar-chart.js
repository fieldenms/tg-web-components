...
import { generateUUID, shadeColor } from '../../reflection/tg-polymer-utils.js';
...
class BarChart {
    ...
    constructor(container, options, data) {
        ...
        this._options = {
            ...
            lines: [],
            barColour: (d, idx) => "#0288D1",
            barLabel: (d, idx) => value(this._options.dataPropertyNames.valueProps[idx], d),
            selectedColour: (d, idx) => shadeColor(this._options.barColour(d, idx), 0.3),
            tooltip: (d, idx) => "<b>" + value(this._options.dataPropertyNames.groupKeyProp, d) + "</b>(<i>" + value(this._options.dataPropertyNames.groupDescProp, d) + "</i>)",
            click: (d, idx) => {},
        };
    ...
    _yDomain() {
        ...
        let maxRange = 0;
        this.data.forEach(d => {
            const valuePropArray = this._options.dataPropertyNames.valueProps.map(valueProp => value(valueProp, d));
            const linePropArray = this._options.lines.map(line => value(line.property, d));
            const newMaxValue = Math.max(
                Math.max(...linePropArray), 
                this._options.mode === BarMode.GROUPED ? Math.max(...valuePropArray) : valuePropArray.filter(value => value > 0).reduce((a, b) => a + b, 0));
            const newMinValue = Math.min(
                Math.min(...linePropArray), 
                this._options.mode === BarMode.GROUPED ? Math.min(...valuePropArray) : valuePropArray.filter(value => value < 0).reduce((a, b) => a + b, 0));
            if (maxRange < newMaxValue) {
                maxRange = newMaxValue;
            }
            if (minRange > newMinValue) {
                minRange = newMinValue;
            }
            
        })
        const factor = (Math.abs(maxRange - minRange) / (this._actualHeight > 30 ? this._actualHeight - 30 : 1)) * 30;
        return [minRange ? minRange - factor : minRange, maxRange + factor];
    }
    ...
    _xScaleSecondary() {
        return d3.scaleBand()
            .rangeRound([0, this._xs.bandwidth()])
            .paddingInner(this._options.mode === BarMode.GROUPED && this._options.dataPropertyNames.valueProps.length > 1 ? 0.1 : 0)// could be configurable
            .domain(this._options.mode === BarMode.GROUPED ? this._options.dataPropertyNames.valueProps.map((val, idx) => idx) : [0]);
    }
